...
class SelfTest(unittest.TestCase):
    ...
    def test_002_mfi_chip(self):
        ...
        try:
            subprocess.check_call(["i2ctransfer", "-y", "0", "r1@0x10"],
        ...
        self.assertEqual(hexdigits,
                         ['0x07', '0x01', '0x03', '0x00', '0x00', '0x00', '0x03', '0x00'])
    ...
    def test_004_spi_nand_badblocks(self):
        ...
        bad_blocks_count = int(re.search('Count of bad physical eraseblocks: +(.+?)\n', output).group(1))
        reserved_blocks_count = int(re.search('Count of reserved physical eraseblocks: +(.+?)\n', output).group(1))
        ...
        self.assertLess(bad_blocks_count, reserved_blocks_count,
                        "Less than 50% of reserved blocks can be bad.")

        self.assertEqual(bad_blocks_count + reserved_blocks_count, 22,
    ...
    def test_005_ram_size(self):
        ...
        meminfo = [line[23:] for line in data.split("\n") if line[15:22] == "Memory:"][0]
        size = int(meminfo.split(" ")[0].split("/")[1].replace('K', ''))
        self.assertTrue(112640 <= size <= 131072, "RAM size between 112'640 and 131'072 kilobytes.")
    ...
    def test_006_u_boot_variables(self):
        ...
        subprocess.check_call(["gardena-provision", '-b', '/tmp/prov-test.json'],
                              stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    ...
    def test_007_factory_data(self):
        ...
        uboot_vars = ("wifiaddr", "ethaddr", "linuxmoduleid", "linuxmodulehqvid", "linuxmoduleunielecid")
        static_vars = {
        ...
        with open(mtdfile, 'rb') as fdata_file:
            ...
            crc_data = fdata_file.read(fdata_end - (fdata_vars['crc'][0] + fdata_vars['crc'][1]))
        crc = zlib.crc32(crc_data)
        crc_bytes = crc.to_bytes(4, byteorder='little')

        # check uboot variables
        for var in uboot_vars:
            self.assertEqual(fdata[var], fw_getenv(var), "Factory data / U-Boot variable mismatch: %s" % var)

        # check static variables
        for var in static_vars:
            self.assertEqual(fdata[var], static_vars[var], "Factory data / static variable mismatch: %s" % key)
    ...
    def test_008_mac_addresses(self):
        ...
        ethernet_mac = \
            subprocess.check_output(ip_cmd + ["eth0"]).decode('ascii').split("\n")[1].strip(' ').split(' ')[1]
        wlan_mac = \
            subprocess.check_output(ip_cmd + ["wlan0"]).decode('ascii').split("\n")[1].strip(' ').split(' ')[1]
        self.assertEqual(ethernet_mac, fw_getenv("ethaddr"), "Ethernet MAC address mismatch")
    ...
    def test_010_kernel(self):
        ...
        self.assertEqual(md5sum, get_batch_config_json()['md5sums']['fitImage-gardena-sg-mt7688.bin'])
    ...
    def test_014_check_sysfs_nand_stats(self):
        ...
        # checks
        self.assertLessEqual(int(values['bad_blocks']), 10) # slightly redundant with test_004_spi_nand_badblocks
    ...
    def test_017_radio_module(self):
        ...
        if not bootstrap_server_present:
            self.skipTest("requires radio module with production test firmware and no ppp running")
        rm_test_fw = RMTestFW()
        ...
        self.assertTrue(production_information['pcba_part_number'] == RMTestFW.PCBAPartNumber.RADIO_MODULE_TYPE_1)
        self.storeValue("radio_module_pcba_part_number", production_information['pcba_part_number'].value)
        self.storeValue("radio_module_pcba_revision", production_information['pcba_revision'])
        self.storeValue("radio_module_pcba_production_time", production_information['pcba_production_time'])
        self.storeValue("radio_module_serial_number", production_information['pcba_serial_number'])
