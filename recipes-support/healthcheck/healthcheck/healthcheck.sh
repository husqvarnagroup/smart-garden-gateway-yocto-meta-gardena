...
test_portcheck_http() {
    ...
    if ! curl --max-time 10 --range 0-1024 -sS "http://${update_url_protocolless}" 2>/dev/null | grep -q "Linux System Firmware for the GARDENA smart Gateway"; then
        result=2
    fi
...
test_portcheck_https() {
    ...
    if ! curl --max-time 10 --range 0-1024 -sS "https://${update_url_protocolless}" 2>/dev/null | grep -q "Linux System Firmware for the GARDENA smart Gateway"; then
        result=2
    fi
